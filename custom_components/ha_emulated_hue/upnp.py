"""SSDP/UPnP discovery responder that mimics a Philips Hue bridge."""
...
class DescriptionXmlView(HomeAssistantView):
    """Serve /description.xml — the UPnP device descriptor Alexa fetches after SSDP."""

    url = "/description.xml"
    name = "ha_emulated_hue:description:xml"
    requires_auth = False

    def __init__(self, advertise_ip: str, advertise_port: int) -> None:
        """Initialize the view."""
...
<URLBase>http://{self.advertise_ip}:{self.advertise_port}/</URLBase>
<device>
<deviceType>urn:schemas-upnp-org:device:Basic:1</deviceType>
<friendlyName>Home Assistant Bridge ({self.advertise_ip})</friendlyName>
...
class UPNPResponderProtocol(asyncio.Protocol):
    """Respond to SSDP M-SEARCH requests advertising as a Hue bridge."""
    ...
    def datagram_received(self, data: bytes, addr: tuple[str, int]) -> None:
        """Respond to M-SEARCH packets."""
        decoded_data = data.decode("utf-8", errors="ignore")

        if "M-SEARCH" not in decoded_data:
            return

        _LOGGER.debug("SSDP M-SEARCH received from %s: %s", addr, decoded_data)
    ...
    def error_received(self, exc: Exception) -> None:
        """Log SSDP errors."""
        _LOGGER.error("SSDP error received: %s", exc)

    def close(self) -> None:
        """Stop the SSDP responder."""
        _LOGGER.info("SSDP responder shutting down")
        if self.transport:
    ...
    def _handle_request(self, decoded_data: str) -> bytes:
        """Choose the appropriate SSDP response."""
    ...
    def _prepare_response(self, search_target: str, unique_service_name: str) -> bytes:
        """Build a pre-encoded SSDP response."""
        # The double newline at the end is required per the SSDP spec
...
async def async_create_upnp_datagram_endpoint(
    ...
    """Create the SSDP socket and protocol."""
